    ...
    <section id="funciones" className="bg-slate-50 px-8 py-16">
      ...
      <div className="grid grid-cols-3 gap-5">
        {features.map((f) => (
          ...
          >
            ...
            <button
              onClick={() => openModal(f.law)}
              className="inline-flex cursor-pointer items-center gap-1 rounded border border-blue-100 bg-blue-50 px-2 py-1 text-xs text-blue-700 transition-colors hover:bg-blue-100"
            >
              ⚖️ {f.law}
            </button>
          </div>
      ...
      {selectedLaw && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="sticky top-0 z-10 px-8 pt-8 pb-4" style={{ background: "linear-gradient(135deg,#0D1B2A 0%,#0D4A75 100%)" }}>
              <button
                onClick={() => setSelectedLaw(null)}
              ...
              {/* Tabs si hay más de un artículo */}
              {articulos.length > 1 && (
                <div className="flex gap-2">
                  {articulos.map((a, i) => (
                    <button
                      ...
                      style={{
                        background: activeTab === i ? "rgba(93,202,165,0.2)" : "rgba(255,255,255,0.06)",
                        border: activeTab === i ? "1px solid rgba(93,202,165,0.4)" : "1px solid rgba(255,255,255,0.1)",
                        color: activeTab === i ? "#5DCAA4" : "rgba(255,255,255,0.5)",
                      }}
                    >
                      {a.num}
                    </button>
              ...
              {articulos[activeTab] && (
                <>
                  <h4 className="mb-3 text-base font-medium text-white">
                    {articulos[activeTab].title}
                  </h4>
                  <p className="mb-5 text-sm font-light leading-relaxed text-white/65">
                    {articulos[activeTab].resumen}
                  </p>
