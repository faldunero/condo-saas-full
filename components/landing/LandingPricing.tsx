const plans = [
  {
    name: "Básico",
    price: "29.900",
    desc: "Para edificios pequeños que comienzan a digitalizarse",
    featured: false,
    features: [
      "Hasta 50 unidades",
      "Gastos comunes digitales",
      "Portal residente",
      "Reportes básicos",
    ],
    btnClass: "border border-blue-600 text-blue-700 hover:bg-blue-50",
    btnLabel: "Comenzar gratis",
  },
  {
    name: "Profesional",
    price: "79.900",
    desc: "La opción completa para administradoras profesionales",
    featured: true,
    features: [
      "Unidades ilimitadas",
      "Asambleas digitales",
      "Fondo de reserva automático",
      "Cobranza integrada Webpay",
      "Cumplimiento Ley 21.442",
    ],
    btnClass: "bg-blue-700 text-white hover:bg-blue-800",
    btnLabel: "Elegir Profesional",
  },
  {
    name: "Enterprise",
    price: null,
    desc: "Para administradoras con múltiples edificios y equipos grandes",
    featured: false,
    features: [
...
export function LandingPricing() {
  return (
    <section id="precios" className="bg-white px-8 py-16">
      ...
      <p className="mb-10 max-w-xl text-base font-light leading-relaxed text-slate-500">
        Sin costos ocultos. Precio por edificio administrado, sin límite de
        unidades en planes superiores.
      </p>
      ...
      <div className="grid grid-cols-3 gap-5">
        {plans.map((p) => (
          ...
          >
            {p.featured && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-blue-700 px-4 py-1 text-xs font-medium text-white whitespace-nowrap">
                Más popular
              </div>
            )}
            ...
            {p.price ? (
              ...
              <p className="mb-1 font-serif text-xl font-semibold text-slate-900">
                Precio a medida
              </p>
            )}
            ...
            <button
              className={`w-full rounded-lg py-3 text-sm font-medium transition-colors ${p.btnClass}`}
            >
              {p.btnLabel}
            </button>
          </div>
